...
class DecoderCBatchNorm(nn.Module):
    ...
    def forward(self, p, c, **kwargs):
        ...
        net = self.fc_p(p)
        #printprint('c shape',c.shape,p.shape)
        net = self.block0(net, c)
        #print('net shape',c.shape,p.shape,net.shape)
...
class PositionParser_graph(nn.Module):    # output 4dof information(center + length) of all parts
    def __init__(self, bottleneck_size=512, pos_dim=4):
        super(PositionParser_graph, self).__init__()
    ...
    def forward(self, x):
        x = x.contiguous().view(1, self.bottleneck_size, -1)    # shape size: [batch_size, feature_size, part_num]
        out = self.layer1(x)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
class PartAssembler(nn.Module):
    ...
    def __init__(self, f_dim=256, img_dim=6, pos_dim=4):
...
class PartAssembler_graph(nn.Module):
    ''' Part Assembly Network class based on graph cnn.
    ...
    def __init__(self, f_dim=256, img_dim=6, pos_dim=4):
        super(PartAssembler_graph, self).__init__()
        self.encoder2d = resnet.ModifiedResnet18(img_dim, f_dim)
        self.encoder3d = VoxelEncoder64(f_dim=f_dim)
        self.decoder = PositionParser_graph(bottleneck_size=f_dim*2, pos_dim=pos_dim)

    def forward(self, imgs, vox):
        # imgs shape: [part_num, 6, 224, 224]
        # vox shape: [part_num, 1, 64, 64, 64]
        feat_img = self.encoder2d(imgs)     # shape: [part_num, f_dim]
        feat_vox = self.encoder3d(vox)      # shape: [part_num, f_dim]
        feat = torch.cat((feat_img, feat_vox), 1)   # shape: [part_num, f_dim*2]
        out = self.decoder(feat)
